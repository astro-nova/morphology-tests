...
def single_galaxy_run(filepath, mag, r_eff, sersic_n, q, n_clumps, sky_mag, psf_fwhm, pxscale=0.396):
    ...
    image_noisy = sky_noise(image_psf, sky_mag, pxscale, telescope_params, transmission_params, bandpass, rms_noise=True)
    ...
    sky_var = ap_sky.do_photometry(image_noisy**2)[0][0]
    var = sky_var + image_perfect
    ...
    inputs = dict(
        mag=mag, r_eff=r_eff, sersic_n=sersic_n, q=q, n_clumps=n_clumps, 
...
if __name__ == '__main__':
    ...
    rs = -1.9*mags + 35 + stats.normal.rvs(loc=0, scale=1.5, size=N)
    rs[rs <= 1] = 1
